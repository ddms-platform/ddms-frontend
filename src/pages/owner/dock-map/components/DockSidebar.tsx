...
const DockSidebar = ({
  ...
  <div className="w-full xl:w-95 bg-ddms-bg-card border border-border p-6 flex flex-col gap-6 shrink-0 shadow-2xl justify-between">
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <label className="text-[11px] font-extrabold tracking-wider text-muted-foreground uppercase flex items-center gap-1">
          <Anchor className="w-3.5 h-3.5 text-ddms-secondary" /> Chọn Bến Neo
          Đậu
        </label>
        <div className="relative">
          <select
            ...
            className="w-full bg-muted border border-border rounded-xl px-4 py-3.5 text-foreground font-semibold text-sm focus:outline-none focus:ring-2 focus:ring-ddms-secondary/50 appearance-none cursor-pointer transition-all"
          >
            {docks.map((dock) => (
              <option
                key={dock.id}
      ...
      <div className="bg-muted/40 border border-border rounded-2xl p-4 flex flex-col gap-3">
        <div className="flex justify-between items-center text-xs">
          ...
          <span className="text-foreground font-bold">{capacity} thuyền</span>
        </div>
        <div className="flex justify-between items-center text-xs">
          <span className="text-muted-foreground font-medium">
            Tàu thuộc sở hữu đang neo
          </span>
          <span className="text-ddms-secondary font-bold">{occupancy} tàu</span>
        </div>

        <div className="mt-1">
          <div className="w-full bg-muted-foreground/20 rounded-full h-2 overflow-hidden border border-border">
            <div
        ...
        <div className="text-[11px] text-muted-foreground font-medium flex justify-between items-center">
      ...
      <div className="flex bg-muted p-1 rounded-xl border border-border">
        <button
          ...
          className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
            activeTab === 'docked'
              ? 'bg-ddms-secondary text-white shadow-md'
      ...
      <div className="overflow-y-auto max-h-90 pr-1 space-y-2.5">
        {activeTab === 'docked' ? (
          activeScheduledBoats.length === 0 ? (
            <div className="text-center py-12 text-xs text-muted-foreground font-medium border border-dashed border-border rounded-2xl">
            ...
            activeScheduledBoats.map((boat) => (
              <div
                ...
                className="bg-muted/40 hover:bg-muted border border-border hover:border-ddms-secondary/40 rounded-xl p-3 flex items-center gap-3 cursor-pointer transition-all duration-200 group shadow-sm"
              >
                <div className="w-10 h-10 rounded-lg bg-ddms-secondary/10 flex items-center justify-center shrink-0 border border-ddms-secondary/15 group-hover:bg-ddms-secondary/20 transition-all">
                  <Anchor className="w-5 h-5 text-ddms-secondary" />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-muted-foreground flex items-center gap-2 mt-0.5">
                    <span className="font-bold text-ddms-secondary bg-ddms-secondary/10 px-1 rounded">
          ...
          otherBoats.map((boat) => {
            ...
            return (
              <div
                ...
                className="bg-muted/30 hover:bg-muted/60 border border-border rounded-xl p-3 flex items-center gap-3 cursor-pointer transition-all duration-200"
              >
    ...
    <div className="pt-4 border-t border-border text-[10px] text-muted-foreground text-center font-medium">
